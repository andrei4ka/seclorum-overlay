...
SRC_URI="http://snapshots.madwifi-project.org/${MY_PV}/${MY_PVR}.tar.gz"
...
src_unpack() {
	...
	epatch "${FILESDIR}"/release_header_fix.patch
	
	if kernel_is gt 2 6 34; then
		epatch "${FILESDIR}"/kernel-2.6.35-if_ath.patch
	fi
...
pkg_postinst() {
	...
	einfo "module."
	einfo
	einfo "To use wifi card in Access Point mode you need add in /etc/modprobe.conf:"
	einfo "alias wifi0 ath_pci"
	einfo "alias ath0 ath_pci"
	einfo "options ath_pci autocreate=ap"
	einfo
