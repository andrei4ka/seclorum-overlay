...
SRC_URI="http://www.imspector.org/downloads/snapshots/${PN}-${SNAPSHOT_DATE}.tar.gz"
...
src_unpack() {
	...
	if use mysql || use postgres || use sqlite || use dbfilter || use dbresponder ; then
	    sed -i -e 's/^#ADD_PLUGINS = .*/ADD_PLUGINS =\$\{ADDON_PLUGINS\}/' Makefile
	fi
...
src_install() {
	...
	dodir -p /var/log/imspector
	dodir -p /share/imspector/contrib
...
pkg_postinst(){
	...
	einfo "under a webserver\'s cgi-bin directory."
}
