...
AWCOMMON_RPM="AWCommon-10.80-13.i686.rpm"
AWCOMMON_SERVER_RPM="AWCommon-server-10.80-13.i686.rpm"
MAYA_RPM="Maya2008_0-2008.0-142.i686.rpm"
MAYA_DOCS_RPM="Maya2008_0-docs_en_US-2008.0-754.i686"

# Patches to download go into SRC_URI
SRC_URI="AWCommon-10.80-13.i686.rpm
	AWCommon-server-10.80-13.i686.rpm
	Maya2008_0-2008.0-142.i686.rpm
	Maya2008_0-docs_en_US-2008.0-754.i686.rpm"
...
src_unpack() {
	...
	rpm_unpack ${DISTDIR}/${AWCOMMON_RPM} || die
	rpm_unpack ${DISTDIR}/${AWCOMMON_SERVER_RPM} || die
	rpm_unpack ${DISTDIR}/${MAYA_RPM} || die

	if use doc ; then
		rpm_unpack ${DISTDIR}/${MAYA_DOCS_RPM} || die
	fi

#(humpback) no shader library on my cd
...
#	cp -a ${CDROM_ROOT}/README.html ${S} || die
	...
	if ! use bundled-libs; then
		...
		#(humpback) I think it is safe on maya7
...
src_install() {
	...
	dosym maya ${AWDIR}/maya

	# The RPM puts these in /usr/local/bin
	dosym ${MAYADIR}/bin/Maya2008 ${MAYADIR}/bin/maya
	...
	# links for pcw
	dosym ${MAYADIR}/lib/libawcsprt.so.1 ${MAYADIR}/lib/libawcsprt.so
	dosym ${MAYADIR}/lib/libpcw_opa.so.1 ${MAYADIR}/lib/libpcw_opa.so
	dosym ${MAYADIR}/lib/libpcwfindkey.so.1 ${MAYADIR}/lib/libpcwfindkey.so
	dosym ${MAYADIR}/lib/libpcwxml.so.1 ${MAYADIR}/lib/libpcwxml.so
...
#	fi
	...
	dosed 's:tail -1: tail -n 1:g' /opt/aw/maya/bin/maya
	...
	# In amd64 with ati drivers we need to set some env vars so we use a wrapper.
	if use amd64; then
		rm -f ${D}/usr/bin/maya
		exeinto /usr/bin
		doexe ${FILESDIR}/maya
	...
	if use doc; then
		fperms 755 /usr/aw/maya/docs/startDocServer.sh
		fperms 755 /usr/aw/maya/docs/shutdownDocServer.sh
		exeinto /etc/init.d
		newexe ${FILESDIR}/maya-docs-r1 maya-docs
	fi

	make_desktop_entry maya "Maya2008" /usr/aw/maya/icons/mayaIcon.xpm
}
