...
SQUID_USER="squid"
SQUID_GROUP="squid"
S=${WORKDIR}/redirector
INSTALLDIR="/opt/rejik"
LOGDIR="/var/log/rejik"
HTTPBAN="http\://seclorum.ru/rejik"
...
src_unpack() {
	...
	cp "${FILESDIR}/check-redirector" "${S}/tools"
	cp "${FILESDIR}/set-permissions" "${S}/tools"
	sed -i -e "s:/usr/local/rejik3:${INSTALLDIR}:g" tools/set-permissions
	sed -i -e "s:/usr/local/rejik3:${INSTALLDIR}:g" tools/check-redirector
	...
	sed -i -e "s:change_log /usr/local/rejik3:change_log ${LOGDIR}:g" redirector.conf.dist
...
src_install() {
	...
	cp "${S}/tools/check-redirector" "${D}${INSTALLDIR}/tools"
	cp "${S}/tools/set-permissions" "${D}${INSTALLDIR}/tools"
	...
	fperms 755 ${INSTALLDIR}/tools/check-redirector
